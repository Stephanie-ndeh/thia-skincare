  ...
  <div class="space-y-5">
    ...
    <template v-else>
      ...
      <div class="space-y-3 sm:hidden">
        <p v-if="codes.length === 0" class="py-10 text-center text-sm text-text-muted">
        ...
        <div
          v-for="code in codes"
          :key="code.id"
          :class="['rounded-xl border border-gray-200 bg-white p-4', isExpired(code) ? 'opacity-60' : '']"
        >
          <div class="flex items-start justify-between gap-3">
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2">
                ...
                <button
                  type="button"
                  class="text-text-muted hover:text-brand-dark"
                  @click="copyCode(code)"
                >
              ...
              <p class="mt-1 text-sm font-medium text-brand-dark">{{ formatDiscount(code) }}</p>
              <div class="mt-1 flex items-center gap-3 text-xs text-text-muted">
                <span>{{ code.usage_limit ? `${code.usage_count} / ${code.usage_limit} uses` : `${code.usage_count} uses` }}</span>
                <span>Expires: {{ formatExpiry(code) }}</span>
              </div>
              <div
                v-if="isExhausted(code)"
            ...
            <div class="flex shrink-0 flex-col items-end gap-2">
              ...
              <div class="flex gap-1">
                <button
                  type="button"
                  class="rounded-lg border border-gray-200 p-1.5 text-text-muted hover:border-brand-dark hover:text-brand-dark"
                  @click="openEdit(code)"
                >
                ...
                <button
                  type="button"
                  class="rounded-lg border border-gray-200 p-1.5 text-text-muted hover:border-red-300 hover:text-red-600"
